...
function framing(mode: CameraMode, trueScale: boolean) {
  ...
  if (mode === "chase") {
    ...
    _look.set(0, 0, 0);
    return { fov: dist > 20 ? 42 : 50, near: Math.max(0.01, visR * 0.35), far: 400 };
  }
  if (mode === "skywatch") {
    ...
    _right.normalize();
    _desired.copy(_delta).multiplyScalar(1.07).addScaledVector(_right, 0.03).addScaledVector(_up, 0.02);
    _look.copy(apoWorld);
  ...
  _right.normalize();
  if (dist > 16) {
    _desired.copy(_sun).multiplyScalar(4.2).addScaledVector(_up, 2.4).addScaledVector(_right, 4.8).addScaledVector(_fwd, -1.2);
  ...
  _desired.copy(apoWorld).multiplyScalar(0.12).addScaledVector(_right, side).addScaledVector(_up, lift).addScaledVector(_sun, 1.5);
  if (_desired.length() < 2.6) _desired.setLength(3.8);
  _look.copy(apoWorld).multiplyScalar(0.35);
  return { fov: dist > 10 ? 42 : 48, near: 0.035, far: 360 };
}
...
export function CameraRig() {
  ...
  return (
    <OrbitControls makeDefault enabled={mode === "free"} enableDamping dampingFactor={0.08} minDistance={1.45} maxDistance={170} enablePan />
  );
}
