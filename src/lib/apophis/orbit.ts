...
export const R_PERI_KM = R_EARTH_KM + ALTITUDE_CA_KM;
...
function perifocalToYUp(x: number, y: number): Vec3 {
  ...
  const eciX = (cosO * cosw - sinO * sinw * cosi) * x + (-cosO * sinw - sinO * cosw * cosi) * y;
  const eciY = (sinO * cosw + cosO * sinw * cosi) * x + (-sinO * sinw + cosO * cosw * cosi) * y;
...
function stateAtSeconds(tSec: number): FlybyState {
  ...
  return {
    position: [positionKm[0] / R_EARTH_KM, positionKm[1] / R_EARTH_KM, positionKm[2] / R_EARTH_KM],
    velocity: [velocity[0] / R_EARTH_KM, velocity[1] / R_EARTH_KM, velocity[2] / R_EARTH_KM],
    radiusKm,
...
export type PathSample = { t: number; position: Vec3; radiusKm: number };
...
export function interpolatePath(timeHours: number): FlybyState {
  ...
  return { ...stateAtHours(t), position };
}
...
export type SimEvent = { id: string; t: number; label: string; detail: string };
export const EVENTS: SimEvent[] = [
  { id: "open", t: -WINDOW_HOURS, label: "T-24h", detail: "Approach window opens" },
  { id: "lunar-in", t: LUNAR_IN_H, label: "Lunar distance", detail: "Closer than the Moon" },
  { id: "geo-in", t: GEO_IN_H, label: "GEO inbound", detail: "Inside geostationary orbit" },
  { id: "ca", t: 0, label: "Closest approach", detail: "31,600 km above the surface" },
  { id: "geo-out", t: GEO_OUT_H, label: "GEO outbound", detail: "Leaving the GEO belt" },
  { id: "lunar-out", t: LUNAR_OUT_H, label: "Lunar distance", detail: "Receding past the Moon" },
  { id: "close", t: WINDOW_HOURS, label: "T+24h", detail: "Approach window closes" },
];

export function moonPosition(timeHours: number): Vec3 {
  const periodH = 27.321661 * 24;
  const theta = 1.15 + (timeHours / periodH) * Math.PI * 2;
  const inc = (5.1 * Math.PI) / 180;
  const r = MOON_DISTANCE_KM / R_EARTH_KM;
  return [r * Math.cos(theta), r * Math.sin(theta) * Math.sin(inc), r * Math.sin(theta) * Math.cos(inc)];
}

export function latLonOfDirection(dir: Vec3, earthAngle: number): { lat: number; lon: number } {
  ...
  return { lat: (Math.asin(y / r) * 180) / Math.PI, lon: (Math.atan2(z, x) * 180) / Math.PI };
}

export function formatUtc(timeHours: number): string {
  const d = new Date(CA_UTC_MS + timeHours * 3600 * 1000);
...
export function formatSignedHours(t: number): string {
  ...
  if (h === 0 && m === 0 && t !== 0) return `T${sign}${s}s`;
  if (h === 0) return `T${sign}${String(m).padStart(2, "0")}m ${String(s).padStart(2, "0")}s`;
